// server.js (ODADA KALMA VE YENİDEN KATILMA MANTIĞI İLE GÜNCELLENMİŞ VERSİYON)
...
io.on('connection', (socket) => {
    ...
    socket.on('join', ({ roomName, role }) => {
        ...
        // --- ODA KATILIMCI MANTIĞI (Joiner) ---
            ...
            // Eğer Kurucu da odadaysa (userCount=1 veya 2), iletişimi başlat.
            if (io.sockets.sockets.has(creatorId)) {
                io.to(creatorId).emit('partnerJoined'); // Kurucuya bilgi
                ...
                // Kurucu yoksa ama kayıtlı (activeRooms'da) ise, Kurucu beklemede kalsın.
    ...
    socket.on('leaveRoom', () => {
        if (currentRoom) {
            ...
            if (isCreator) {
                // Kurucu ayrılırsa: Odayı tamamen temizle ve herkese haber ver.
                activeRooms.delete(roomToLeave);
                // Kendisi dışındaki herkese sinyal gönder
                socket.to(roomToLeave).emit('partnerDisconnected'); 
                console.log(`[${socket.id}] KURUCU Manuel ayrıldı. "${roomToLeave}" odası TEMİZLENDİ.`);
            } else {
                // Katılımcı ayrılırsa: Sadece Kurucuya haber ver (oda temizlenmez).
    ...
    socket.on('disconnect', () => {
        if (currentRoom) {
            ...
            if (isCreator) {
                // Kurucu koparsa: Odayı tamamen temizle ve herkese haber ver.
                activeRooms.delete(roomToLeave);
                // Kendisi dışındaki herkese sinyal gönder
                socket.to(roomToLeave).emit('partnerDisconnected'); 
                console.log(`[${socket.id}] KURUCU KOPTU. "${roomToLeave}" odası TEMİZLENDİ.`);
            } else {
                // Katılımcı koparsa: Sadece Kurucuya haber ver (oda AKTİF KALIR).
                socket.to(roomToLeave).emit('partnerLeft', { socketId: socket.id });
